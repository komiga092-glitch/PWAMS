...
# IMPORTANT: Get the NEW CSRF token after login (it changes!)
$csrf = ($sv.Cookies.GetCookies($baseUrl) | Where-Object { $_.Name -eq 'pwams_csrf' }).Value
...
$payload = @{
    ...
    nic_passport = 'NID_CFLT2_' + $uniqueTag
    email = 'conflict2_' + $uniqueTag + '@example.com'
    phone = '1234567890'
    address = '123 Test St'
...
# Step 5: Attempt to update with stale version (version 1, but server has version 2) - Client A
...
$payload3 = @{
    full_name = 'Updated By Client A (Stale)'
    nic_passport = 'NID_CFLT2_' + $uniqueTag
    email = 'conflict2_' + $uniqueTag + '@example.com'
    phone = '1111111111'
