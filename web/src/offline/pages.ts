...
declare global {
  interface Window {
    pwamsOfflineReadStore?: <T>(store: OfflineStoreName) => Promise<T[]>;
  }
}
...
type PageConfig = {
  ...
  columns: string[];
  values: (record: OfflineRecord) => string[];
};
...
function setLocalIndicator(): void {
...
function pageEntity(): string | undefined {
  ...
  if (path.startsWith("/loan-repayments")) return "loan_repayments";
  return undefined;
}
...
async function renderOfflineList(config: PageConfig): Promise<void> {
  const records = (
    await listOfflineRecords<OfflineRecord>(config.store)
    ...
    .map(
      (record) =>
        `<tr>${config
          ...
          .map(
            (value) =>
              `<td>${value.startsWith("<a ") ? value : escapeHTML(value)}</td>`,
          )
          .join("")}</tr>`,
    )
    .join("");
...
function renderOfflineDetail(record: OfflineRecord): void {
...
async function renderOfflinePage(): Promise<void> {
  ...
  if (!id) return;
  const records = await listOfflineRecords<OfflineRecord>(config.store);
  const record = records.find((item) => item.id === id && !item.is_deleted);
  if (record) renderOfflineDetail(record);
  else
    document
      .querySelector("main")
      ?.insertAdjacentHTML(
        "afterbegin",
        `<div class="empty">No offline data available.</div>`,
      );
}
