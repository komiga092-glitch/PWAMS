...
function getStudentFormData(form: HTMLFormElement): CreateStudentRequest {
...
function validateStudentData(data: CreateStudentRequest): string | null {
  if (!data.person_id) {
...
async function createStudent(data: CreateStudentRequest): Promise<void> {
  if (!navigator.onLine) {
  ...
  if (!response.ok) {
    const message =
      result.message || result.error || "Failed to create student.";
...
async function loadPersons(): Promise<void> {
  ...
  try {
    ...
    const result = (await response.json()) as PersonsResponse;
...
studentModal?.addEventListener("click", (event: MouseEvent) => {
...
studentForm?.addEventListener("submit", async (event: SubmitEvent) => {
  ...
  const submitButton = studentForm.querySelector(
    'button[type="submit"]',
  ) as HTMLButtonElement | null;
