...
async function runSyncPendingMutations() {
    ...
    pending.sort((left, right) => order.indexOf(left.entityType) - order.indexOf(right.entityType));
    for (const mutation of pending) {
        ...
        try {
            ...
            if (response.status === 409) {
                ...
                await updateMutationStatus(mutation.id, "CONFLICT", result?.message ?? "Synchronization conflict");
                window.dispatchEvent(new CustomEvent("pwams:sync-conflict"));
                continue;
            }
            if (response.status >= 400 && response.status < 500) {
                const result = await response.json().catch(() => null);
                await updateMutationStatus(mutation.id, "FAILED", result?.message ?? "Synchronization failed");
                continue;
            }
