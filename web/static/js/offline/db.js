...
export async function getOfflineSessionOwner() {
    const database = await openOfflineDatabase();
    return new Promise((resolve, reject) => {
        const request = database
            .transaction(OFFLINE_STORES.metadata, "readonly")
        ...
        request.onsuccess = () => {
            const value = request.result;
            resolve(typeof value?.username === "string" && value.username.length > 0
                ? value.username
                : null);
        };
        request.onerror = () => reject(request.error ?? new Error("Unable to read offline session owner"));
    });
}
export async function setOfflineSessionOwner(username) {
    const database = await openOfflineDatabase();
    await new Promise((resolve, reject) => {
        const transaction = database.transaction(OFFLINE_STORES.metadata, "readwrite");
        transaction
        ...
        transaction.oncomplete = () => resolve();
        transaction.onerror = () => reject(transaction.error ?? new Error("Unable to store offline session owner"));
    });
}
/**
...
export async function clearOfflineIdentity() {
    ...
    await new Promise((resolve, reject) => {
        ...
        for (const store of stores) {
            if (store === OFFLINE_STORES.metadata)
                continue;
            transaction.objectStore(store).clear();
        }
        transaction.oncomplete = () => resolve();
        transaction.onerror = () => reject(transaction.error ?? new Error("Unable to clear offline identity"));
    });
}
