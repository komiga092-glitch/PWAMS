"use strict";
const donorEditForm = document.getElementById("donor-edit-form");
donorEditForm?.addEventListener("submit", async (event) => {
    ...
    const data = {
        ...
        contact_person_phone: String(formData.get("contact_person_phone") ?? "").trim(),
        preferred_donation_type: String(formData.get("preferred_donation_type") ?? "").trim(),
    ...
    // UPDATE DONOR
    // =========================
    const updateButton = document.getElementById("donor-update-btn");
    try {
        if (updateButton) {
...
//# sourceMappingURL=donor-edit.js.map